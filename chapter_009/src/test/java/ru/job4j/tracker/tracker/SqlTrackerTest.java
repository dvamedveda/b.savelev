...
public class SqlTrackerTest {
    ...
    public void whenAddingItemThenAddingIt() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            ...
            assertThat(sqlTracker.findByName("Тестовая заявка").get(0).getName(), is("Тестовая заявка"));
    ...
    public void whenUpdateItemThenItUpdating() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            ...
            sqlTracker.replace(String.valueOf(item.getId()), newItem);
            Item foundedItem = sqlTracker.findById(String.valueOf(item.getId()));

            assertThat(foundedItem.getName(), is(newItem.getName()));
    ...
    public void whenDeletingItemThenItDeleting() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            ...
            sqlTracker.delete(String.valueOf(addedItem.getId()));
            assertNull(sqlTracker.findById(String.valueOf(addedItem.getId())));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void whenExistOneMoreItemsThenFindAll() {
        ...
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            int idFirst = sqlTracker.add(itemOne).getId();
            int idSecond = sqlTracker.add(itemTwo).getId();
            int idThirst = sqlTracker.add(itemThree).getId();
            ...
            assertThat(result.get(1).getName(), is(itemTwo.getName()));
            ...
            assertThat(result.get(2).getName(), is(itemThree.getName()));
    ...
    public void whenFindingByIdThenFindingIt() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            Item item = new Item("Тестовая заявка", "Тестовое описание заявки", 12345L);
            int id = sqlTracker.add(item).getId();
            assertThat(id, is(sqlTracker.findById(String.valueOf(id)).getId()));
            assertThat(item.getName(), is(sqlTracker.findById(String.valueOf(id)).getName()));
            assertThat(item.getDescription(), is(sqlTracker.findById(String.valueOf(id)).getDescription()));
            assertThat(item.getCreated(), is(sqlTracker.findById(String.valueOf(id)).getCreated()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
