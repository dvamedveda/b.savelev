...
public class StubInputTest {
    ...
    public void whenChooseAddItemThenAddingIt() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            ...
            assertThat(addedItem.getName(), is("Тестовая заявка номер 1."));
    ...
    public void whenUpdatingItemThenUpdateIt() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            ...
            Input input = new StubInput(new String[]{"2", String.valueOf(updatingItem.getId()), "Заявка 2", "Описание 2", "0"});
            StartUI ui = new StartUI(sqlTracker, input);
            ui.startWork();
            Item updatedItem = sqlTracker.findAll().get(0);
            assertThat(updatedItem.getName(), is("Заявка 2"));
    ...
    public void whenDeleteItemThenItDeleting() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            int id = sqlTracker.add(new Item("Заявка 1", "Описание 1", 123L)).getId();
            Input input = new StubInput(new String[]{"3", String.valueOf(id), "0"});
            StartUI ui = new StartUI(sqlTracker, input);
            ui.startWork();
    ...
    public void whenSearchByIdAndHaveTwoItemsThenFindOne() {
        try (SqlTracker sqlTracker = new SqlTracker(ConnectionRollback.create(this.init()))) {
            sqlTracker.add(new Item("Заявка 1", "Описание 1", 123L));
            Item firstItem = sqlTracker.findAll().get(0);
            Input input = new StubInput(new String[]{"6", String.valueOf(firstItem.getId()), "0"});
            this.loadByteOut();
            StartUI ui = new StartUI(sqlTracker, input);
            ui.startWork();
            String expected = new StringBuilder()
